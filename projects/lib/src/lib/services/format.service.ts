...
export class FormatService {
  ...
  formatDate(
    value: any,
    options: DateTimeFormatOptions & { format?: any } = {}
  ): string {
  ...
  formatNumber(
    value: any,
    options: NumberFormatOptions & { format?: any } = {}
  ): string {
    const { formats, locale } = this.intlService.getConfig();
    const { format } = options;
  ...
  formatPlural(value: any, options: PluralFormatOptions = {}): string {
    ...
    try {
      return formatters.getPluralFormat
        .bind(null)(locale, filteredOptions)
        .format(value);
    } catch (e) {
  ...
  formatMessageAsync(
    ...
    return Observable.create((observer: Observer<undefined | string>) => {
      this.intlService.getAsync(id).subscribe((message: string) => {
  ...
  private processFormatMessage(
    ...
    const { formats, locale, defaultLocale } = this.intlService.getConfig();
  ...
  private escapeValues(rawValues: { [k: string]: any } = {}) {
    ...
    return Object.keys(rawValues).reduce<{ [k: string]: any }>(
      (escaped, name) => {
